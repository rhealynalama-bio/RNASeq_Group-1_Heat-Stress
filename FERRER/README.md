...
10. **Were all RNA-seq samples in your group similar in quality? Explain.** Mostly, but not entirely. All four samples passed per-base and per-sequence quality scores, adapter content, and overrepresented sequences, and all shared the same per base sequence content failure — likely tied to library prep rather than any one run's fault. GC content varied more between conditions than within them: the two control samples (SRR7758331 39%, SRR7758332 41%) sat close together, while the two treatment samples (SRR7758328 45%, SRR7758330 41–42%) ran higher, hinting at an expression shift under heat stress. Read counts also varied widely, from about 1.3M (SRR7758332) to nearly 5M (SRR7758328), reflecting differences in sequencing depth rather than quality per se.
